...
public class PagingVO implements Serializable {
	public static void main(String[] args) {
		...
		pagingVO.setCurPage(20);
		pagingVO.setTotalRowCount(2368); // DB에서 SELECT count(*)
	...
	public void pageSetting() {
		...
		if(curPage == (totalRowCount / rowSizePerPage+1) ) {
			lastRow = totalRowCount;
		}else {
			lastRow = firstRow -1 + rowSizePerPage;
		}
		totalPageCount = totalRowCount / pageSize + 1;		
		firstPage = curPage-1 - (curPage-1) % pageSize + 1;
		if(curPage > totalPageCount - totalPageCount % pageSize) {
			lastPage = curPage - (curPage % pageSize) + (totalPageCount % pageSize);
		}else {
			lastPage = firstPage -1 + pageSize;
		}
	}
	 @Override 
